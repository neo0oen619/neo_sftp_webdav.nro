...
namespace CONFIG
{
    ...
    void LoadConfig()
    {
        ...
        for (int i = 0; i < sites.size(); i++)
        {
            ...
            // Provide sensible defaults for the first two sites if they
            // haven't been configured yet.
            if (setting.server[0] == '\0')
            {
                if (i == 0)
                {
                    // Default SFTP endpoint exposed via Tailscale Funnel.
                    snprintf(setting.server, sizeof(setting.server),
                             "sftp://unk1server.tail544f02.ts.net:2022");
                }
                else if (i == 1)
                {
                    // Default WebDAV endpoint exposed via Tailscale Funnel.
                    snprintf(setting.server, sizeof(setting.server),
                             "webdavs://unk1server.tail544f02.ts.net/dav");
                }
    ...
    {
        if (strncmp(setting->server, "sftp://", 7) == 0)
        {
            setting->type = CLIENT_TYPE_SFTP;
        }
        else if (strncmp(setting->server, "smb://", 6) == 0)
        {
        ...
        {
            // Treat typical WebDAV endpoints (like /dav) as WebDAV,
            // otherwise fall back to generic HTTP server type.
            const char *dav = strstr(setting->server, "/dav");
            if (dav != nullptr)
                setting->type = CLIENT_TYPE_WEBDAV;
            else
