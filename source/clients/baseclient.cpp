...
int BaseClient::Connect(const std::string &url, const std::string &username, const std::string &password)
{
    ...
    client->SetBasicAuth(this->http_username, this->http_password);
    // Many home/self-hosted WebDAV/HTTP endpoints (including Tailscale Funnel)
    // use certificates that are not present in the bundled CA store for libcurl
    // on Switch. To avoid spurious "SSL peer certificate" errors and allow
    // connecting to such servers, disable strict peer verification here.
